...
After it's done, get the IP address of your instance:
    ...
    IP=$(bin/get-ip-address-of-instance.sh instance)
...
get the name of the stamp file, execute `bin/print-stamp-path.sh`. Whenever you
successfully logged in, and the file exists, the transformation finished:

    bin/wait-until-done.sh $IP matekey.priv
...
After these steps, run:
    ...
    bin/test-rs.sh $STAGING_VM_URL $VM_POSTFIX
